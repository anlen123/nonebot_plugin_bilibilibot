...
ALL_PERMISSION = GROUP_ADMIN | GROUP_OWNER | PRIVATE_FRIEND | SUPERUSER | GROUP_MEMBER

follow_liver_command = on_command("关注主播", permission=ALL_PERMISSION)


@follow_liver_command.handle()
async def follow_liver_command_handler(event: Union[PrivateMessageEvent, GroupMessageEvent],
...
async def unfollow_liver_command_handler(event: Union[PrivateMessageEvent, GroupMessageEvent],
                                         args: Message = CommandArg()):
    uid_list = args.extract_plain_text().split()
    await create_user(event)
    if isinstance(event, PrivateMessageEvent):
...
async def listFollowingCommandHandler(event: Union[PrivateMessageEvent, GroupMessageEvent],
                                      args: Message = CommandArg()):
    await create_user(event)
...
async def followTelegramCommandHandler(event: Union[PrivateMessageEvent, GroupMessageEvent],
                                       args: Message = CommandArg()):
    await create_user(event)
...
async def unfollowTelegramCommandHandler(event: Union[PrivateMessageEvent, GroupMessageEvent],
                                         args: Message = CommandArg()):
    await create_user(event)
...
followTelegramByShare = on_message(
    rule=regex(
        '^\[CQ:json[\w\W]*"appid":100951776[\w\W]*www.bilibili.com\/bangumi\/play\/[\w\W]*') & privateMessageRule,
    permission=PRIVATE_FRIEND
)
